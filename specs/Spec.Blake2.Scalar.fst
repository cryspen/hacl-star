...
inline_for_extraction
unfold let state (a:alg) = lseq (word_t a) 16

inline_for_extraction
unfold let iv (a:alg) = lseq (word_t a) 8
...
(* Functions *)
let g1 (a:alg) (wv:state a) (i:idx_t) (j:idx_t) (r:rotval (wt a)) : Tot (state a) =
  wv.[i] <- (wv.[i] ^. wv.[j]) >>>. r

let g2 (a:alg) (wv:state a) (i:idx_t) (j:idx_t) (x:word_t a) : Tot (state a) =
  wv.[i] <- (wv.[i] +. wv.[j] +. x)

let g2z (a:alg) (wv:state a) (i:idx_t) (j:idx_t) : Tot (state a) =
  wv.[i] <- (wv.[i] +. wv.[j])

let sigma (i:size_nat{i < 12}) (j:idx_t) : idx_t =
  size_v sigmaTable.[16*i + j]
...
let blake2_mixing_ al wv a b c d x y =
  let rt = rTable al in
...
[@"opaque_to_smt"]
let blake2_mixing = blake2_mixing_
...
let blake2_round_ al m i wv =
  let wv = blake2_mixing al wv 0 4 8 12 m.[sigma i 0] m.[sigma i 1] in
  let wv = blake2_mixing al wv 1 5 9 13 m.[sigma i 2] m.[sigma i 3] in
  let wv = blake2_mixing al wv 2 6 10 14 m.[sigma i 4] m.[sigma i 5] in
  let wv = blake2_mixing al wv 3 7 11 15 m.[sigma i 6] m.[sigma i 7] in
  
  let wv = blake2_mixing al wv 0 5 10 15 m.[sigma i 8] m.[sigma i 9] in
  let wv = blake2_mixing al wv 1 6 11 12 m.[sigma i 10] m.[sigma i 11] in
  let wv = blake2_mixing al wv 2 7 8 13 m.[sigma i 12] m.[sigma i 13] in
  let wv = blake2_mixing al wv 3 4 9 14 m.[sigma i 14] m.[sigma i 15] in
  wv

[@"opaque_to_smt"]
let blake2_round = blake2_round_
...
let blake2_init_hash a kk nn =
  ...
  let iv0' = iv0 ^. s0 in
  [@inline_let]
  let l_iv = [iv0';iv1;iv2;iv3;iv4;iv5;iv6;iv7;
              iv0;iv1;iv2;iv3;iv4;iv5;iv6;iv7] in
  assert_norm(List.Tot.length l_iv == 16);
  let s_iv = of_list l_iv in
  s_iv
