...
bool print_test1(uint8_t* in, int in_len, uint8_t* exp256, uint8_t* exp512){
  ...
  digestif_sha256_update(&ctx,in,in_len);
  digestif_sha256_finalize(&ctx,comp256);
...
int main()
{
  ...
  for (int j = 0; j < ROUNDS; j++) {
    struct sha256_ctx ctx;
    digestif_sha256_init(&ctx);
    digestif_sha256_update(&ctx,plain,SIZE);
    digestif_sha256_finalize(&ctx,plain);
  }
